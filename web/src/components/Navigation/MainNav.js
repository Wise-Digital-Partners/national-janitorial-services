...
const StyledMainNav = styled.nav`
  ${({ navigationStyle }) =>
    navigationStyle === 'hidden' ? tw`hidden` : null};
  ${({ headerHasBorder }) =>
    headerHasBorder
  ...
  @media (max-width: 1023px) {
    ...
    /* ${({ navigationStyle }) =>
      navigationStyle === 'overlap' ? tw`bg-white` : null};  */
  }
  &[data-fixed='true'] {
  ...
  >
    ...
    {menuItem.submenu && (
      <ul className='submenu'>
        {menuItem.submenu.slice().sort((a, b) => a.title.localeCompare(b.title)).map(subMenuItem => (
  ...
  {menuItems.map(menuItem => (
    <li key={menuItem.id} className='relative mb-8'>
      {menuItem.title === 'Services' ? (
        <a href="'/commercial-cleaning-company/" onClick={clickHandler}>
          {menuItem.title}
        </a>
