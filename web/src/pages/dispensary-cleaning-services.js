import React from "react";
import { graphql } from "gatsby";
import { GatsbyImage } from "gatsby-plugin-image";

import Layout from "../components/Layout";
import SearchEngineOptimization from "../components/SEO";
import HeroStacked from "../components/Hero/HeroStacked";
import Testimonials from "../components/Repeating/Testimonials";
import Clients from "../components/Repeating/Clients";
import WhyUs from "../components/Repeating/WhyUs";
import CallToAction from "../components/Repeating/CTA";
import CovidSplit from "../components/Repeating/CovidSplit";

const Page = ({ data }) => {
   return (
      <Layout navigationStyle="standard" headerLinkColor="" headerHasBorder={false}>
         <SearchEngineOptimization
            title="Dispensary Cleaning Services | Janitorial Company Long Beach"
            description="For dispensary cleaning services in the Long Beach area, see why Long Beach Janitorial is a top choice. Learn more & schedule a consultation!"
            openGraphImage={data.openGraphImage}
            twitterOpenGraphImage={data.twitterOpenGraphImage}
         />

         <HeroStacked
            image={data.heroStacked}
            backgroundFixed={true}
            imageMaxHeight="max-h-[468px]"
            heading="Dispensary Cleaning Services"
            subtext="Keeping your dispensary clean and green."
            textMaxWidth="max-w-4xl"
         />

         <section className="py-16 md:py-32">
            <div className="container">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-y-12 md:gap-x-10 lg:gap-x-20 items-center">
                  <div>
                     <h2>Top Dispensary Cleaning Services in Long Beach</h2>
                     <p className="mb-0">
                        As your inventory grows, keeping your facility clean and disinfected should be the least of your worries. Let Long Beach Janitorial provide a thorough cleaning of your dispensary and cannabis facility so your customers can browse and
                        purchase without worry. Our on-site services help ensure that your dispensary’s cleanliness is always a cut above the rest.
                     </p>
                  </div>
                  <div>
                     <GatsbyImage image={data.intro} alt="Top Dispensary Cleaning Services in Long Beach" />
                  </div>
               </div>
            </div>
         </section>

         <section className="bg-gray-50 py-14 md:py-18">
            <div className="container">
               <h3>Dispensary Areas We Clean</h3>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-x-12">
                  <div>
                     <p className="md:mb-0">
                        At LBJ, we conduct site visits and work closely with you to develop a cleaning plan. Our certified cleaning experts handle
                        dusting, wiping, vacuuming, disinfection, and more for the following areas.
                     </p>
                  </div>
                  <div>
                     <ul className="styled-list">
                        <li>Product shelves</li>
                        <li>Air filters</li>
                        <li>Offices</li>
                     </ul>
                  </div>
                  <div>
                     <ul className="styled-list">
                        <li>Bathrooms</li>
                        <li>Trash disposal</li>
                        <li>Walls and floors</li>
                     </ul>
                  </div>
               </div>
            </div>
         </section>

         <CovidSplit className="pt-16 md:pt-32 mb-16 md:mb-32" />

         <Clients className="pb-16 md:pb-32" headingLevel="h2" />

         <Testimonials headingLevel="h2" />

         <WhyUs className="py-16 md:py-32" headingLevel="h2" />

         <CallToAction headingLevel="h2" />
      </Layout>
   );
};

export const query = graphql`
   {
      openGraphImage: file(relativePath: { eq: "open-graph/facebook/Dispensaries_FB.jpg" }) {
         publicURL
      }
      twitterOpenGraphImage: file(relativePath: { eq: "open-graph/twitter/Dispensaries_TW.jpg" }) {
         publicURL
      }
      heroStacked: file(relativePath: { eq: "industries/dispensaries/1.0 Dispensaries Hero Desktop.jpg" }) {
         childImageSharp {
            gatsbyImageData(layout: FULL_WIDTH, placeholder: BLURRED, quality: 100)
         }
      }
      intro: file(relativePath: { eq: "industries/dispensaries/2.0 dispens cleaning.jpg" }) {
         childImageSharp {
            gatsbyImageData(layout: FULL_WIDTH, placeholder: BLURRED, quality: 100)
         }
      }
   }
`;
export default Page;
